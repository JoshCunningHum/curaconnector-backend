...
import { getConversation } from './messages.get.mjs';
...
import '../_/message.mjs';

const recipients_get = defineEventHandler(async (event) => {
  ...
  const recipients = await Promise.all(
    (await getConversation(user.id)).map(async (m) => ({
      ...m.with,
      preferences: (await db.select().from(users).where(eq(users.id, m.with.id))).at(0).preferences
    }))
  );
  return recipients;
});

export { recipients_get as default };
//# sourceMappingURL=recipients.get.mjs.map
