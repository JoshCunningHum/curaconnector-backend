...
const verify_post = defineEventHandler(async (event) => {
  var _a;
  const sid = parseInt((_a = getRouterParam(event, "id")) != null ? _a : "-1");
  const [session] = await db.select().from(visitSessions).where(eq(visitSessions.id, sid));
  ...
  if (session.recipient !== user.id) {
    throw createError({
      ...
      message: "Only the visit session recipient can verify their own visit session"
    });
  }
  ...
  await db.update(visitSessions).set({ isVerified: true, verifiedDate: (/* @__PURE__ */ new Date()).toISOString() }).where(eq(visitSessions.id, sid));
});
