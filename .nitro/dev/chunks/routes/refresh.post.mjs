...
import { count, eq } from 'file://D:/Documents/WebsitesPrograms/Work/Axel/backend/node_modules/.pnpm/drizzle-orm@0.44.2_@types+b_8a624cc9ffd98e02c10a3d5dfc6bcc4d/node_modules/drizzle-orm/index.js';
...
import { k as refreshTokens, g as safeAwait, m as decodeToken, e as db, f as users, i as createAccessToken, J as JWT_EXPIRES_IN } from '../_/nitro.mjs';
...
const refresh_post = defineEventHandler(async (event) => {
  ...
  const [{ count: usercount }] = await db.select({ count: count() }).from(users).where(eq(users.id, decoded.id));
  if (usercount === 0) {
    throw createError({
      statusCode: 403,
