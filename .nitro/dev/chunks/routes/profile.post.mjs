...
const profile_post = defineEventHandler(async (event) => {
  ...
  if (!isObjectEmpty(body.preference)) {
    ...
    await db.update(users).set({ preferences: appliedPrefs }).where(eq(users.id, user.id));
  }
  if (!isObjectEmpty(body.common)) {
    await db.update(users).set(body.common).where(eq(users.id, user.id));
  }
  await db.update(users).set({ updatedAt: dayjs().toISOString() }).where(eq(users.id, user.id));
});
