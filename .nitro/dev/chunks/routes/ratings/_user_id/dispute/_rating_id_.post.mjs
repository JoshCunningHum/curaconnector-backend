...
import { and, eq, count } from 'file://D:/Documents/WebsitesPrograms/Work/Axel/backend/node_modules/.pnpm/drizzle-orm@0.44.2_@types+b_8a624cc9ffd98e02c10a3d5dfc6bcc4d/node_modules/drizzle-orm/index.js';
import { e as db } from '../../../../_/nitro.mjs';
import { r as ratings, a as ratingDisputes } from '../../../../_/ratings.mjs';
...
const _rating_id__post = defineEventHandler(async (event) => {
  ...
  const [rating] = await db.select().from(ratings).where(and(eq(ratings.id, +ratingId), eq(ratings.to, user.id)));
  if (!rating) {
    throw createError({
      statusCode: 404,
  ...
  const [existingDispute] = await db.select({ value: count() }).from(ratingDisputes).where(eq(ratingDisputes.ratingId, +ratingId));
  if (existingDispute.value > 0) {
    throw createError({ statusCode: 409, message: "This rating has already been disputed." });
  }
  const [newDispute] = await db.insert(ratingDisputes).values({
    ratingId: +ratingId,
...
export { _rating_id__post as default };
//# sourceMappingURL=_rating_id_.post.mjs.map
