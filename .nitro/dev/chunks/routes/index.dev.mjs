...
import { g as getUser, e as execBasedUserType, r as rosterproviders, c as companies, b as recipients, p as providers } from '../_/getUser.mjs';
import { U as UserNotFoundError } from '../_/error.mjs';
import { getTableColumns, eq, sql, or, and } from 'file://D:/Documents/WebsitesPrograms/Work/Axel/backend/node_modules/.pnpm/drizzle-orm@0.44.2_@types+b_8a624cc9ffd98e02c10a3d5dfc6bcc4d/node_modules/drizzle-orm/index.js';
import { e as db, f as users } from '../_/nitro.mjs';
import { m as messages } from '../_/message.mjs';
import { r as rosterAccess } from '../_/rosteraccess.mjs';
...
const index_dev = defineEventHandler(async (event) => {
...
const getConveration = async (userID) => {
  ...
  const _msgquery = await db.select({
    with: {
      id: users.id,
      name: sql`CASE
  ...
  }).from(users).leftJoin(companies, eq(users.id, companies.userId)).leftJoin(recipients, eq(users.id, recipients.userId)).leftJoin(providers, eq(users.id, providers.userId)).leftJoin(rosterproviders, eq(users.id, rosterproviders.id)).innerJoin(msg, eq(users.id, msg.with)).where(eq(msg.row, 1));
  return _msgquery;
};
...
async function getMessagesRoster(user) {
}
async function getMessagesCommon(user) {
  ...
  db.select().from(rosterAccess).where(
    and(
