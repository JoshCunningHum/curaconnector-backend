...
import { g as getUser, e as execBasedUserType, r as rosterproviders } from '../_/getUser.mjs';
import { U as UserNotFoundError } from '../_/error.mjs';
import { eq, inArray, getTableColumns, and } from 'file://D:/Documents/WebsitesPrograms/Work/Axel/backend/node_modules/.pnpm/drizzle-orm@0.44.2_@types+b_8a624cc9ffd98e02c10a3d5dfc6bcc4d/node_modules/drizzle-orm/index.js';
...
const index_get = defineEventHandler(async (event) => {
  ...
  const filter = (_a = body.with) != null ? _a : [];
  const user = await getUser(event);
  if (!user) throw UserNotFoundError();
  const sessions = (_b = await execBasedUserType(user, {
...
const _recipientSessions = async (from, user) => {
  const filter_user = eq(visitSessions.recipient, user.id);
  const specify_provider = inArray(visitSessions.visitor, from);
  return await db.select().from(visitSessions).where(
    from.length > 0 ? and(filter_user, specify_provider) : specify_provider
  );
};
const _providerSessions = async (from, user) => {
  const filter_user = eq(visitSessions.visitor, user.id);
  const specify_recipient = inArray(visitSessions.recipient, from);
  return await db.select().from(visitSessions).where(
    from.length > 0 ? and(filter_user, specify_recipient) : specify_recipient
  );
};
const _companySessions = async (from, user) => {
