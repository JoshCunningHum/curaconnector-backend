...
import { g as getUser, p as providers, r as rosterproviders } from '../_/getUser.mjs';
...
const favorites_get = defineEventHandler(async (event) => {
  ...
  const favs = await db.select().from(favorites).leftJoin(users, eq(favorites.to, users.id)).leftJoin(providers, eq(providers.userId, favorites.to)).leftJoin(rosterproviders, eq(rosterproviders.userId, favorites.to)).where(eq(favorites.by, user.id));
  return favs.map((r) => ({
    user: r.users,
